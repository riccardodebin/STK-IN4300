...
w <- 1/abs(beta.LS[-1])^1
set.seed(123)
cross.validation.result <- cv.glmnet(x = X, y = y, alpha = 1, penalty.factor = w)
lambda.cv <- cross.validation.result$lambda.min
mod.garrote <- glmnet(y = y, x = X, lambda = lambda.cv, alpha = 1, penalty.factor = w)
round(rbind(y.mean, mod.garrote$beta), 3)


### elastic-net ###
findLambda <- function(x, y, a)
{
  tmp <- cv.glmnet(x, y, family = 'gaussian', alpha = a, standardize = FALSE)
  which.min(tmp$cvm)
  c(min(tmp$cvm), tmp$lambda.min)
}
set.seed(123)
cv <- sapply(seq(0, 1, by = 0.05), findLambda, x = X, y = y)
par <- list(alpha = NULL, lambda = NULL, choice = NULL)
par$choice <- which.min(cv[1, ])
par$alpha <- seq(0, 1, by = 0.05)[par$choice]
par$lambda <- cv[2, par$choice]
model.elnet <- glmnet(x = X, y = y, family = 'gaussian', alpha = par$alpha, standardize = FALSE,
                      lambda = par$lambda)
round(rbind(y.mean, model.elnet$beta), 3)


### adaptive lasso ###
findGamma <- function(x, y, g)
{
  w <- 1/abs(beta.LS[-1])^g
  set.seed(123)
  tmp <- cv.glmnet(x = X, y = y, alpha = 1, penalty.factor = w)
  which.min(tmp$cvm)
  c(min(tmp$cvm), tmp$lambda.min)
}
set.seed(123)
cv <- sapply(seq(0, 5, by = 0.05), findGamma, x = X, y = y)
par <- list(gamma = NULL, lambda = NULL, choice = NULL)
par$choice <- which.min(cv[1, ])
par$gamma <- seq(0, 5, by = 0.05)[par$choice]
par$lambda <- cv[2, par$choice]
w <- 1/abs(beta.LS[-1])^par$gamma
mod.adaLasso <- glmnet(y = y, x = X, lambda = par$lambda, alpha = 1, penalty.factor = w)
round(rbind(y.mean, mod.adaLasso$beta), 3)
...
set.seed(123)
cv <- cv.ncvreg(X = X, y = y, penalty = "SCAD", gamma = 3.7, alpha = 1)
mod.scad <- ncvreg(X = X, y = y, penalty = "SCAD", gamma = 3.7, alpha = 1, lambda = cv$lambda.min)
beta.scad <- mod.scad$beta[ ,mod.scad$lambda == cv$lambda.min]
round(mod.scad$beta, 3)
...
train.x <- as.matrix(data[data$train == TRUE, -c(9,10)])
y.new <- data$lpsa[data$train == FALSE]
X.new <- as.matrix(data[data$train == FALSE, -c(9,10)])
names <- colnames(X.new)
p <- ncol(X)
scale(X.new)
X.new <- sapply(1:p, function(i,xx,m,s) (xx[, i] - m[i])/s[i],
                xx = X.new, m = colMeans(train.x), s = apply(train.x, 2, sd))
X.new <- cbind(1, X.new)
X <- cbind(1, X)
...
# ls
train.error$ls <- mean((y - X%*%beta.LS)^2)
test.error$ls <- mean((y.new - X.new%*%beta.LS)^2)
# best subset
train.error$bs <- mean((y - X[, names(model.bestSubset.AIC$coefficients)]%*%
                          model.bestSubset.AIC$coefficients)^2)
test.error$bs <- mean((y.new - X.new[, names(model.bestSubset.AIC$coefficients)]%*%
                         model.bestSubset.AIC$coefficients)^2)
# ridge
train.error$ridge <- mean((y - X%*%beta.ridge)^2)
test.error$ridge <- mean((y.new - X.new%*%beta.ridge)^2)
# lasso
train.error$lasso <- mean((y - X%*%c(y.mean, as.vector(mod.lasso$beta)))^2)
test.error$lasso <- mean((y.new - X.new%*%c(y.mean, as.vector(mod.lasso$beta)))^2)
# pcr
train.error$pcr <- mean((y - X%*%c(y.mean, mod.pcr$coefficients[, , 6]))^2)
test.error$pcr <- mean((y.new - X.new%*%c(y.mean, mod.pcr$coefficients[, , 6]))^2)
# pls
train.error$pls <- mean((y - X%*%c(y.mean, mod.pls$coefficients[, , 6]))^2)
test.error$pls <- mean((y.new - X.new%*%c(y.mean, mod.pls$coefficients[, , 6]))^2)
# non-negative garrote
train.error$nng <- mean((y - X%*%c(y.mean, as.vector(mod.garrote$beta)))^2)
test.error$nng <- mean((y.new - X.new%*%c(y.mean, as.vector(mod.garrote$beta)))^2)
# elastic-net
train.error$enet <- mean((y - X%*%c(y.mean, as.vector(model.elnet$beta)))^2)
test.error$enet <- mean((y.new - X.new%*%c(y.mean, as.vector(model.elnet$beta)))^2)
# adaptive lasso
train.error$ada <- mean((y - X%*%c(y.mean, as.vector(mod.adaLasso$beta)))^2)
test.error$ada <- mean((y.new - X.new%*%c(y.mean, as.vector(mod.adaLasso$beta)))^2)
# SCAD
train.error$scad <- mean((y - X%*%mod.scad$beta)^2)
test.error$scad <- mean((y.new - X.new%*%mod.scad$beta)^2)
